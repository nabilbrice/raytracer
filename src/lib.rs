...
pub fn raytrace(ray: &Ray, scene: &Vec<Hittable>, scatter_depth: u8, rng: &mut impl Rng) -> Color {
    ...
    let obj_relative_loc: Vec3 = match &hit_obj.shape {
        ...
        Shape::Disc(disc) => scatter_loc - disc.centre,
        Shape::Cylinder(cylinder) => (scatter_loc - cylinder.centre).normalize(),
        Shape::TruncCone(cone) => (scatter_loc - cone.centre).normalize(),
    };
    hit_obj.material.spectrum(&obj_relative_loc) * raytrace(&scatter_ray, scene, scatter_depth - 1, rng)
...
pub fn logspace<const N: usize>(start: f64, stop: f64) -> [f64;N] {
    ...
    for (i, element) in array.iter_mut().enumerate().take(N) {
        *element = start * multiplier.powi(i as i32);
    }
    array
}